...
async function createUser(data) {
        ...
        ...(data.user.hasPosts && {
            posts: {
                create: {
                    ...
                    content: 'Database architecture is very crutial in designing a performant and scalable system',
    ...
    const user = await lib_1.prisma.user.upsert({
    ...
    for (const postInput of data.posts ?? []) {
        const { comments: commentFields = [], ...postData } = postInput;
        postData.user = { connect: { id: user.id } };
        const postExist = await lib_1.prisma.post.findFirst({
            where: {
